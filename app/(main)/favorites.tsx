...
export default function FavoritesScreen() {
  const [segment, setSegment] = useState<(typeof SEGMENTS)[number]["id"]>("favorites");
      ...
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.segmentRow}>
          {SEGMENTS.map((item) => (
            <TouchableOpacity
              key={item.id}
              style={[styles.segmentBtn, segment === item.id && styles.segmentActive]}
              onPress={() => setSegment(item.id)}
            >
              <Text
                style={[styles.segmentText, segment === item.id && styles.segmentTextActive]}
              >
                {item.label}
              </Text>
            </TouchableOpacity>
          ...
          <View>
            {FAVORITE_RECIPES.map((recipe) => (
              <TouchableOpacity key={recipe.id} style={styles.recipeCard}>
                <ImageBackground source={recipe.hero} style={styles.recipeHero} imageStyle={styles.recipeHeroImage}>
                  ...
                  <View style={styles.heroTopRow}>
                    ...
                    <TouchableOpacity onPress={() => handleShare(recipe)} style={styles.heroShare}>
                  ...
                  <View style={styles.heroContent}>
                    ...
                    <View style={styles.heroStats}>
                      <View style={styles.heroStatItem}>
                        ...
                        <Text style={styles.heroStatText}>{recipe.rating.toFixed(1)}</Text>
                      </View>
                      <View style={styles.heroStatItem}>
                        <Ionicons name="bookmark-outline" size={14} color="#fff" />
                        <Text style={styles.heroStatText}>{recipe.saves.toLocaleString()}</Text>
                      </View>
                    </View>
                  </View>
                </ImageBackground>
                  ...
                  <View style={styles.collectionBody}>
                    ...
                    <View style={styles.collectionFooter}>
                      <View style={styles.collectionCount}>
                        ...
                        <Text style={styles.collectionCountText}>{collection.recipes} recipes</Text>
                      </View>
                      <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  heroTagText: { color: "#fff", fontWeight: "600", marginLeft: 8, fontSize: 12 },
  heroShare: {
    width: 36,
    height: 36,
    borderRadius: 12,
  ...
  collectionTitle: { color: "#fff", fontSize: 16, fontWeight: "700", lineHeight: 22 },
  ...
  collectionCountText: { marginLeft: 6, color: "#fff", fontWeight: "600", fontSize: 12 },
  collectionShare: {
    width: 32,
    height: 32,
    borderRadius: 12,
